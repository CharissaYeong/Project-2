...
function SearchEntry({...props}) {
  ...
  const handleLike = async (entryId) => {
    try {
      ...
      await axios.post(`http://localhost:3001/entries/${entryId}/like`, {userId: localStorage.getItem('userID')});
  
      // Get the updated number of likes for the entry
      const response = await axios.get(`http://localhost:3001/entries/${entryId}/likes/number/count`);
